...
# New augmentation: RandomPitchShift

class RandomPitchShift:
    def __init__(self, sampling_rate, max_steps=5):
        self.sampling_rate = sampling_rate
        self.max_steps = max_steps

    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        pitch_shift = random.randint(-self.max_steps, self.max_steps)
        # Convert PyTorch tensor to numpy array
        x_np = x.numpy()
        # Apply pitch shift
        shifted_wave_np = librosa.effects.pitch_shift(x_np, sr=self.sampling_rate, n_steps=pitch_shift)
        # Convert numpy array back to PyTorch tensor of the same type
        shifted_wave = torch.from_numpy(shifted_wave_np).to(x.dtype)
        return shifted_wave

class RandomTimeStretch:
    def __init__(self, min_rate=0.8, max_rate=1.25):
    ...
    def __call__(self, wave: torch.Tensor) -> torch.Tensor:
        stretch_rate = np.random.uniform(self.min_rate, self.max_rate)
        # Convert the torch.Tensor to numpy array and ensure it is a float32 array
        wave = wave.numpy().astype('float32')
        # Apply time stretch
        stretched_wave = librosa.effects.time_stretch(wave, stretch_rate)
        # Convert the numpy array back to torch.Tensor
        return torch.from_numpy(stretched_wave)
