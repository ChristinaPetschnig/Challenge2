...
class ESC50(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Apply new augmentations for training subset

        if self.subset == "train":

            #ply transformations 
            random_pitch_shift = transforms.RandomPitchShift(sampling_rate=config.sr)
            #random_time_stretch = transforms.RandomTimeStretch(min_rate=0.8, max_rate=1.2)
            random_noise = transforms.RandomNoise(min_noise=0.0, max_noise=0.05) # You can adjust the noise range
            random_scale = transforms.RandomScale(max_scale=1.25) # You can adjust the scaling factor

            # Convert wave to a tensor before applying transformations
            #wave_tensor = torch.from_numpy(wave)
            wave_tensor = torch.from_numpy(wave).float()  # Make sure wave_tensor is a floating-point tensor

            # Apply transformations
            # Apply transformations
            wave_tensor = random_pitch_shift(wave_tensor)
            #wave_tensor = random_time_stretch(wave_tensor)
            wave_tensor = random_noise(wave_tensor)
